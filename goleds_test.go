...
func TestApp_FullInitialiseAndShutdown(t *testing.T) {
	testYaml := `
Hardware:
  Display:
    LedsTotal: 20
    ForceUpdateDelay: 100ms
  Sensors:
    SmoothingSize: 3
    LoopDelay: 50ms
    Calibration:
...
`
	tempDir := t.TempDir()
	cfile := filepath.Join(tempDir, "config.yml")
	if err := os.WriteFile(cfile, []byte(testYaml), 0644); err != nil {
	...
	// Initialize in TUI mode (realp=false, sensp=false)
	if err := app.initialise(cfile, false, false); err != nil {
		t.Fatalf("App.initialise failed: %v", err)
	}

	// Verify producers were created
	if _, ok := app.ledproducers[NightLedUID]; !ok {
		t.Errorf("Expected NightLED producer to be initialized")
	}
	if _, ok := app.ledproducers[ClockLedUID]; !ok {
		t.Errorf("Expected ClockLED producer to be initialized")
	}
	if _, ok := app.ledproducers[CylonLedUID]; !ok {
		t.Errorf("Expected CylonLED producer to be initialized")
	}
